"""
Обновление самого релея: узнать, что вышла новая версия, и попросить хост
её поставить.

Копия одноимённого модуля панели с той же схемой общения с хостовым
агентом (файлы update-request.json / update-result.json / agent.json в
каталоге данных): агент один на оба сервиса и не различает, кто его
просит. Отличия — образ, состав компонентов и источник настроек (у релея
нет объекта settings, канал берётся из окружения).
...
Само обновление релей не делает: контейнер не может перезапустить себя,
а давать веб-сервису docker.sock — значит отдать ему root на хосте. Вместо
этого он кладёт файл-запрос в каталог данных, а хостовый агент
(noisefloor install-agent, ставится установщиком) по нему делает pull и
up, проверяет здоровье и при неудаче откатывается. Результат агент пишет
рядом — релей его показывает.
"""
...
IMAGE = "vbif666/noisefloor-vless-reality"
...
def components() -> dict[str, str]:
    ...
    try:
        xray = Path("/opt/XRAY_VERSION").read_text().strip() or "?"
    except OSError:
...
def _channel() -> str:
    value = (os.environ.get("UPDATE_CHANNEL") or "stable").strip().lower()
...
def request_update() -> tuple[bool, str]:
    """Попросить хост обновить релей. Возвращает (ok, сообщение)."""
    state = status()
    ...
    return True, ("Запрос отправлен. Релей перезапустится через полминуты; каскад с первого "
                  "сервера и его клиенты переподключатся сами.")


def run(stop_event: threading.Event) -> None:
    """Фоновая проверка раз в CHECK_INTERVAL_SECONDS. Первая — через минуту
    после старта, чтобы не мешать подъёму xray."""
